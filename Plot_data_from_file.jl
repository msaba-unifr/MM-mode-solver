...
plot_data = real.(field[3,:,:])

plt = heatmap(collect(-img_yrange/2 : res : img_yrange/2),collect(-2*img_zrange/4 : res : 2*img_zrange/4),plot_data,aspect_ratio=:equal,color=:phase,right_margin=5mm)
plot(plt,title = string("___, mode: ",mode,", ",freq," THz"))
